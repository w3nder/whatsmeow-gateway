...
func TestGroupInfoUnknownGroupIsNotFound(t *testing.T) {
	fake := newFakeWAClient()
	fake.markPaired()
	conn, cancel, runErrCh := setupGroupGateway(t, fake, "channel-groups")
	defer shutdownStatusRoundtripGateway(t, cancel, runErrCh)
	...
	if reply["ok"] != false || reply["error"].(map[string]any)["code"] != "not_found" || reply["error"].(map[string]any)["message"] != "group not found (404)" {
